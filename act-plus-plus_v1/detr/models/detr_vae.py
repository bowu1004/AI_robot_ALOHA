...
class DETRVAE(nn.Module):
    ...
    def __init__(self, backbones, transformer, encoder, state_dim, num_queries, camera_names, vq, vq_class, vq_dim,
        ...
        """
        super().__init__()
        self.num_queries = num_queries  # 检测目标的数量
        self.camera_names = camera_names  # 相机名称
        self.transformer = transformer  # transformer模型
        self.encoder = encoder  # 编码器
        self.vq, self.vq_class, self.vq_dim = vq, vq_class, vq_dim  # Q 矢量量化
        self.state_dim, self.action_dim = state_dim, action_dim  # 状态维度 动作维度
        hidden_dim = transformer.d_model  # 隐藏层维度 512
        self.action_head = nn.Linear(hidden_dim, action_dim)  # 动作头部
        self.is_pad_head = nn.Linear(hidden_dim, 1)  # 填充
        self.query_embed = nn.Embedding(num_queries, hidden_dim)  # q 查询潜入？？
        #
        if backbones is not None:
            # 将backbones输出的特征映射到模型的隐藏维度（hidden_dim）
            self.input_proj = nn.Conv2d(backbones[0].num_channels, hidden_dim, kernel_size=1)
            # 将多个backbones模型 保存在容器中
            self.backbones = nn.ModuleList(backbones)
            # 将即机器人状态--->连接到隐藏层 线性层
            self.input_proj_robot_state = nn.Linear(state_dim, hidden_dim)
        else:
            # input_dim = 14 + 7 # robot_state + env_state
            # 创建几个线性层 用于将机器人状态和环境状态映射到隐藏层
            self.input_proj_robot_state = nn.Linear(state_dim, hidden_dim)
        ...
        self.cls_embed = nn.Embedding(1, hidden_dim)  # 创建一个嵌入层 1---> hidden_dim向量
    ...
    def encode(self, qpos, actions=None, is_pad=None, vq_sample=None):
        ...
        if self.encoder is None:
            ...
            if is_training:  # 训练模式
                ...
                # at 嵌入的维度为（bs , seq + 2 , hidden_dim）
                encoder_input = torch.cat([cls_embed, qpos_embed, action_embed], axis=1)  # (bs, seq+1, hidden_dim)
                encoder_input = encoder_input.permute(1, 0, 2)  # (seq+1, bs, hidden_dim) Q 调整维度
                # do not mask cls token
                # Q 填充两列false  is_pad的作用是什么？
                cls_joint_is_pad = torch.full((bs, 2), False).to(qpos.device)  # False: not a padding
                is_pad = torch.cat([cls_joint_is_pad, is_pad], axis=1)  # (bs, seq+1)
                # obtain position embedding
                pos_embed = self.pos_table.clone().detach()  # 位置嵌入 1 * k+2 * hidden_dim
                pos_embed = pos_embed.permute(1, 0, 2)  # (seq+1, 1, hidden_dim)
                # query model
                encoder_output = self.encoder(encoder_input, pos=pos_embed, src_key_padding_mask=is_pad)
                encoder_output = encoder_output[0]  # take cls output only
                latent_info = self.latent_proj(encoder_output)  # 最后转成64维
    ...
    def forward(self, qpos, image, env_state, actions=None, is_pad=None, vq_sample=None):
        ...
        """
        # 先进入encode函数  latent_input潜在输入 probs概率None = binaries二进制None mu均值 logvar方差
        ...
        if self.backbones is not None:
            ...
            for cam_id, cam_name in enumerate(self.camera_names):
                ...
                pos = pos[0]
                all_cam_features.append(self.input_proj(features))  # 卷积
        ...
        a_hat = self.action_head(hs)  # 512 * 14
...
def build(args):
    ...
    if args.no_encoder:
        ...
        # 创建编码器层 采用transformer的编码器层结构 4层 8头
