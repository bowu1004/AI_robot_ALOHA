...
class ACTPolicy(nn.Module):
    ...
    def __call__(self, qpos, image, actions=None, is_pad=None, vq_sample=None):
        ...
        if actions is not None:  # training time
            # at num_queries的含义是什么？ ===> num_queries是指每个时间步的动作数？？？
            actions = actions[:, :self.model.num_queries]
            is_pad = is_pad[:, :self.model.num_queries]

            loss_dict = dict()
            # 进入CVAE模块 其中a_hat是预测的动作 probs是动作的概率分布 binaries是动作的二值化 mu和logvar是动作的均值和方差
            a_hat, is_pad_hat, (mu, logvar), probs, binaries = self.model(qpos, image, env_state, actions, is_pad,
                                                                          vq_sample)
            if self.vq or self.model.encoder is None:
                total_kld = [torch.tensor(0.0)]
            else:
                total_kld, dim_wise_kld, mean_kld = kl_divergence(mu, logvar)  # 计算KL散度的损失
            if self.vq:
                loss_dict['vq_discrepancy'] = F.l1_loss(probs, binaries, reduction='mean')
            all_l1 = F.l1_loss(actions, a_hat, reduction='none')  # 均方误差 重构误差
            l1 = (all_l1 * ~is_pad.unsqueeze(-1)).mean()
            loss_dict['l1'] = l1
            loss_dict['kl'] = total_kld[0]
            # CVAE的损失函数 为l1损失和KL散度的加权和 l1为动作的均方误差 KL散度为动作的均值和方差
            loss_dict['loss'] = loss_dict['l1'] + loss_dict['kl'] * self.kl_weight
            return loss_dict
        else:  # inference time 推理模式  a_hat 输出的动作
