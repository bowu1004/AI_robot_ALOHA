...
ros::CallbackQueue queue_jointPos;
ros::CallbackQueue queue_other;
...
void MoveJ_PO_Callback(const dual_arm_msgs::MoveJ_PO msg)
{
    int res = 0;
    byte speed;
...
void JointPos_Callback(const dual_arm_msgs::JointPos msg)
{
    ...
    if (res == 0)
    {
        ...
        // ROS_INFO_STREAM("[JointPos_Callback thread=" << boost::this_thread::get_id() << "]");
    }
    else
    {
...
{
    // ROS_INFO_STREAM("[Get_Arm_Joint thread=" << boost::this_thread::get_id() << "]");
    // ROS_INFO("Get_Arm_Joint timer_callback!\n");
    timer_cnt++;
...
int main(int argc, char **argv)
{
    ...
    nh_.setCallbackQueue(&queue_jointPos);
    // subscriber
    JointPos_Cmd = nh_.subscribe("JointPos", 200, JointPos_Callback);

    nh_.setCallbackQueue(&queue_other);
	...
	sub_MoveJ_PO_Cmd = nh_.subscribe("MoveJ_PO_Cmd", 10, MoveJ_PO_Callback);
     ...
     * *********************************************************************************/
    ROS_INFO("subscribe Lift_SetHeight!\n");
     ...
     * *********************************************************************************/
    ROS_INFO("subscribe Lift_SetSpeed!\n");
    ...
    ros::AsyncSpinner spinner_jointPos(2, &queue_jointPos);
    spinner_jointPos.start();

    ros::AsyncSpinner spinner_other(3, &queue_other);
    spinner_other.start();
    ...
    // ROS_INFO_STREAM("[socket read thread=" << boost::this_thread::get_id() << "]");
