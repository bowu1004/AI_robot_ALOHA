...
# e = IPython.embed/home/xyc/interbotix_ws/src/mobile-aloha-noted/aloha_scripts/record_episodes.py
...
class ImageRecorder:
    def __init__(self, init_node=True, is_debug=False):
        ...
        self.camera_names = ['cam_high', 'cam_left_wrist',
                             'cam_right_wrist']  # ['cam_high', 'cam_low', 'cam_left_wrist', 'cam_right_wrist']
        if init_node:
            rospy.init_node('rm_image_recorder', anonymous=True)
    ...
    def image_cb(self, cam_name, data):
        # 设置对象属性 属性不存在可创建
        # bridge.imgmsg_to_cv2 可以将ros图片消息转换成opencv图片 方便后续的学习和训练
        setattr(self, f'{cam_name}_image', self.bridge.imgmsg_to_cv2(data, desired_encoding='passthrough'))
        # setattr(self, f'{cam_name}_image',
        #         cv2.cvtColor(self.bridge.imgmsg_to_cv2(data, desired_encoding='passthrough'), cv2.COLOR_BGR2RGB))
    ...
    def get_images(self):
        image_dict = dict()
        for cam_name in self.camera_names:
            # getattr(obs,name[,default]) 获取对象的属性
...
class Recorder:
    def __init__(self, side, init_node=True, is_debug=False):
        ...
        print(f"/{side}_arm/joint_states")
        rospy.Subscriber(f"/{side}_arm/joint_states", JointState, self.puppet_state_cb)
